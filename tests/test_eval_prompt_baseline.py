...
- ``select_few_shot_examples`` deterministically picks complete + sparse +
  ...
  its input (the data-leakage gate that protects the comparison).
...
def test_strong_prompt_constraints_match_spec():
    """The CONSTRAINTS block must include the four required points:
    JSON-only output, null for missing fields, ISO date format, and an
    empty list for parties when none are identified."""
