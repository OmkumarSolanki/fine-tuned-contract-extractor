"""Shared utilities for the Phase 5 baseline evaluators.
...
# Must match training/prepare_dataset.py:USER_PROMPT_TEMPLATE so the Phase 5
# baselines can recover the raw contract text from a wrapped user message.
...
def extract_contract_text(user_message_content: str) -> str:
    ...
    The training pipeline prepends a fixed prefix; Phase 5 baselines need the
    raw body so they can wrap it in their own prompts. If the prefix is absent
    (e.g. malformed input), returns the input unchanged.
    """
...
def make_prediction_record(contract_id: str, raw_output: str) -> dict:
    """Build the canonical Phase 5 prediction record.
...
def write_predictions(records: list[dict], path: Path | str) -> None:
    ...
    Phase 5 outputs are JSON, not JSONL, so they can be diffed and inspected
    with ``jq`` end-to-end. The directory is created on demand.
    """
    path = Path(path)
